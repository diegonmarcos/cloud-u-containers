...
) -> Optional[google_service_account.Credentials]:
    """Return service account credentials with domain-wide delegation, or None if not configured."""
    key_path = os.getenv("GOOGLE_SERVICE_ACCOUNT_KEY_PATH")
    if not key_path or not os.path.isfile(key_path):
        return None
    subject = subject_email or os.getenv("USER_GOOGLE_EMAIL")
    logger.info(
        f"Using service account credentials, impersonating {subject}"
    )
    return google_service_account.Credentials.from_service_account_file(
        key_path, scopes=required_scopes, subject=subject
    )
...
def get_credentials(
    ...
    """
    # Short-circuit: use service account credentials if configured
    sa_creds = _get_service_account_credentials(required_scopes, user_google_email)
...
async def get_authenticated_google_service(
    ...
    is_service_account = isinstance(credentials, google_service_account.Credentials)

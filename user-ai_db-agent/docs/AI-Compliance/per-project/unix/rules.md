...
> `~/git/cloud-infra-desktop/` — NixOS host config, home-manager (desktop + termux), system flakes

## Project-Specific BLOCKs

| Pattern | Message |
|---------|---------|
| `nixos-rebuild switch` directly | Use `~/git/cloud-infra-desktop/aa_nixos-surface_host/build.sh switch` |
| `home-manager switch` directly | Use `build.sh switch` from the flake directory |
| `nix-on-droid switch` directly | Use `~/git/cloud-infra-desktop/bb_flakes_termux/build.sh switch` |
...
| Flake | Command |
|-------|---------|
| NixOS host | `~/git/cloud-infra-desktop/aa_nixos-surface_host/build.sh` (options: r/b/t) |
| Desktop home-manager | `~/git/cloud-infra-desktop/ba_flakes_desktop/build.sh switch` |
| Termux home-manager | `~/git/cloud-infra-desktop/bb_flakes_termux/build.sh switch` |

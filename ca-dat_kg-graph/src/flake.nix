{
  ...
  outputs = { self, nixpkgs }: let
    ...
    # Single source of truth: build-surrealdb.json (symlink → 2_configs/dist/
    ...
    mkDockerCompose = pkgs: pkgs.writeText "docker-compose.yml" ''
      ...
      # ╠══════════════════════════════════════════════════════════════════╣
      # ║ Source: ~/git/cloud/a_solutions/ca-dat_kg-graph/src/flake.nix   ║
      # ║ Rebuild: ~/git/cloud/a_solutions/ca-dat_kg-graph/build.sh ship  ║
      # ╚══════════════════════════════════════════════════════════════════╝
      services:
        surrealdb:
          image: ${config.image}
  ...
  in {
    ...
    in let
      defaultPkg = pkgs.runCommand "kg-graph-configs" {} ''
        mkdir -p $out

...
async function finOpsAssets(): Promise<string> {
  ...
  sections.push("\n── CLOUDFLARE DNS ──");
  const cfCandidates = ["/app/_cloud-data-consolidated.json", join(CLOUD_DATA_DIR, "..", "cloud", "9_others", "dist", "_cloud-data-consolidated.json"), join(CLOUD_DATA_DIR, "_cloud-data-consolidated.json")];

...
#                                 host follows the bind; and the MCP block is
#                                 EXACTLY what gen-mcp-tpl.sh produces (a
#                                 hand edit fails before any live probe runs)
...
# The MCP servers block is a marked REGION generated by cloud-u-linux
# da_my-ai/data/claude/gen-mcp-tpl.sh from cloud-infra's derived server list,
# filtered by mcp-policy.json's container-goose platform to
# cloud-cgc-pub-mcp + cloud-infra-mcp. This is the assertion from the
# container's side: the committed copy is exactly what the generator produces
# today. cloud-infra's lint-pipeline runs the same --check from its side and
# skips this platform when cloud-u-containers is absent; here the two repos are
# siblings and cloud-infra's dist/ is what has to be present. The failure this
# replaces: a hand-written list that drifted from the fleet's, #184, which left
# a headless agent consulting a server its client was never offered.

...
import { getConfig, getServiceFolder, getServiceDir, getVmSshAlias, getServicesForVm, resolveVmId, } from "../config.js";
import { CONFIG_PATH, SSH_CONFIG_PATH, SOLUTIONS_DIR, FRONT_DIR, RUST_API_MESH, RUST_API_PUBLIC, } from "../utils/paths.js";
                ...
                for (const entry of entries) {
                    ...
                    if (existsSync(buildJson)) {
                        try {
                            ...
                            const mods = (config.build ?? []).map((b) => b.mod).join(", ");
                            lines.push(`| ${entry} | ${config.framework ?? "vanilla"} | ${config.port ?? "-"} | ${mods || "none"} | ${category} |`);
                        }
                        catch { /* skip */ }
                    }
                }
...
`;
    ...
    // ── Resource Templates ──
    server.resource("service-detail", new ResourceTemplate("cloud://services/{name}", {
        list: async () => ({
    ...
    server.resource("vm-detail", new ResourceTemplate("cloud://vms/{vm_id}", {
        list: async () => ({

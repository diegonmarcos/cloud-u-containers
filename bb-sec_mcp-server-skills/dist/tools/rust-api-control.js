...
export function registerRustApiControlTools(server) {
    // ── VM Control (3 tools) ──
    server.tool("vm_start", "Start a VM via the Rust API (handles OCI/GCP abstraction)", {
    ...
    server.tool("vm_stop", "Stop a VM gracefully via the Rust API", {
        vm: z.string().describe("VM ID or SSH alias"),
    }, async ({ vm }) => {
        const vmId = resolveVmId(vm);
    ...
    server.tool("vm_reset", "Reset/force-restart a VM via the Rust API", {
        vm: z.string().describe("VM ID or SSH alias"),
    }, async ({ vm }) => {
        const vmId = resolveVmId(vm);
    ...
    server.tool("container_start", "Start a container via the Rust API (preferred — handles VM auto-wake and validation).", {
        vm: z.string().describe("VM ID or SSH alias"),
        name: z.string().describe("Container name"),
    }, async ({ vm, name }) => {
        const vmId = resolveVmId(vm);
        const result = rustApiPost(`/api/vms/${encodeURIComponent(vmId)}/containers/${encodeURIComponent(name)}/start`, undefined, 60_000);
        audit("container_start", `${name}@${vm}`, result.ok ? "OK" : `FAILED (HTTP ${result.status})`);
        return formatResult(`Container start ${name}@${vm}`, result);
    });
    server.tool("container_stop", "Stop a container on a VM", {
        vm: z.string().describe("VM ID or SSH alias"),
        name: z.string().describe("Container name"),
    }, async ({ vm, name }) => {
        const vmId = resolveVmId(vm);
        const result = rustApiPost(`/api/vms/${encodeURIComponent(vmId)}/containers/${encodeURIComponent(name)}/stop`, undefined, 30_000);
        audit("container_stop", `${name}@${vm}`, result.ok ? "OK" : `FAILED (HTTP ${result.status})`);
        return formatResult(`Container stop ${name}@${vm}`, result);
    });
    server.tool("container_restart", "Restart a container on a VM", {
        vm: z.string().describe("VM ID or SSH alias"),
        name: z.string().describe("Container name"),
    }, async ({ vm, name }) => {
        const vmId = resolveVmId(vm);
        const result = rustApiPost(`/api/vms/${encodeURIComponent(vmId)}/containers/${encodeURIComponent(name)}/restart`, undefined, 60_000);
    ...
    server.tool("service_start", "Start all containers for a service on a VM", {
        vm: z.string().describe("VM ID or SSH alias"),
        service: z.string().describe("Service name"),
    }, async ({ vm, service }) => {
        const vmId = resolveVmId(vm);
        const result = rustApiPost(`/api/vms/${encodeURIComponent(vmId)}/services/${encodeURIComponent(service)}/start`, undefined, 120_000);
        audit("service_start", `${service}@${vm}`, result.ok ? "OK" : `FAILED (HTTP ${result.status})`);
        return formatResult(`Service start ${service}@${vm}`, result);
    });
    server.tool("service_stop", "Stop all containers for a service on a VM", {
        vm: z.string().describe("VM ID or SSH alias"),
        service: z.string().describe("Service name"),
    }, async ({ vm, service }) => {
        const vmId = resolveVmId(vm);
        const result = rustApiPost(`/api/vms/${encodeURIComponent(vmId)}/services/${encodeURIComponent(service)}/stop`, undefined, 60_000);
        audit("service_stop", `${service}@${vm}`, result.ok ? "OK" : `FAILED (HTTP ${result.status})`);
        return formatResult(`Service stop ${service}@${vm}`, result);
    });
}
//# sourceMappingURL=rust-api-control.js.map
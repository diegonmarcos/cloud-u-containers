...
function validatePath(repo, path) {
    const repoBase = REPOS[repo];
    if (!repoBase)
        throw new Error(`Unknown repo: ${repo}. Valid: ${VALID_REPOS.join(", ")}`);
...
export function registerRepoTools(server) {
    server.tool("read_file", "Read a file from a repository (cloud, unix, vault, front, tools)", {
    ...
    server.tool("search_repos", "Search (grep) across repositories for a pattern", {
    ...
    server.tool("list_directory", "List directory contents in a repository", {
        repo: z.enum(["cloud", "unix", "vault", "front", "tools"]).describe("Repository name"),

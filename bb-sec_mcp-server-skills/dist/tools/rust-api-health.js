...
export function registerRustApiHealthTools(server) {
    // ── Health (7 tools) ──
    server.tool("health_alive", "Check if the Rust API is alive (heartbeat)", {}, async () => {
    ...
    server.tool("health_deployed", "List live deployed containers on VMs (probes via SSH/Docker)", {
    ...
    server.tool("health_status", "Full health dashboard — declared + deployed + drift combined", {
        vm: z.string().optional().describe("Filter by VM ID or alias (omit for all VMs)"),
    }, async ({ vm }) => {
        const vmId = vm ? resolveVmId(vm) : undefined;
    ...
    server.tool("profile_container", "Run diagnostic profile: CPU, memory, network, disk, ports, health, processes, uptime", {
    ...
    server.tool("profile_vm", "Batch profile all containers on a VM", {
    ...
    server.tool("service_get_info", "Get single service metadata including spec URL", {
    ...
    server.tool("service_get_spec", "Fetch the full OpenAPI/Swagger spec for a service", {
        service: z.string().describe("Service name"),
    }, async ({ service }) => {
        const result = rustApiGet(`/api/services/${encodeURIComponent(service)}/spec`, 30_000);
        if (!result.ok) {
            return {
                content: [{ type: "text", text: `No spec available for ${service}: ${result.error ?? "unknown error"}\n${result.raw}` }],
                isError: true,
            };
        }
        const text = typeof result.data === "string" ? result.data : JSON.stringify(result.data, null, 2);
        ...
        return {
            content: [{ type: "text", text: `OpenAPI spec for ${service}:\n\n${truncated}` }],
        };
    });
    server.tool("service_discover_all", "Parallel-fetch all service specs (cached 5min server-side)", {}, async () => {
        ...
        return {
            content: [{ type: "text", text: `All service specs:\n\n${truncated}` }],
        };
    });
}
//# sourceMappingURL=rust-api-health.js.map
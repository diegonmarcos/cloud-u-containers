...
export function registerNativeOpsTools(server) {
    server.tool("build_ship", "Run full deployment pipeline: build → secrets → deploy → compose (build.sh ship)", {
    ...
    server.tool("build_docker", "Build and push Docker image for a service (build.sh docker)", {
    ...
    server.tool("secrets_status", "Show secrets encryption status for services", {
    ...
    server.tool("backup_trigger", "Trigger backup for a service's data (borg=media files, bup=general files, db=database dump)", {
...
//# sourceMappingURL=native-ops.js.map
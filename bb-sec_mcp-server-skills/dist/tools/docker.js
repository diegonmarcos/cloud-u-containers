...
export function registerDockerTools(server) {
    server.tool("docker_ps", "List Docker containers on a VM", {
    ...
    server.tool("docker_control", "Start/stop/restart a container via SSH. Use for debugging or when Rust API is down. Prefer container_start/stop/restart for normal operations.", {
        vm: z.string().describe("VM ID or SSH alias"),
    ...
    server.tool("docker_logs", "Get Docker container logs. since format: '1h', '30m', '2024-01-01'", {
        vm: z.string().describe("VM ID or SSH alias"),
        container: z.string().describe("Container name"),
    ...
    server.tool("docker_compose_up", "Recreate all containers for a service from its compose file on the VM. Does NOT rebuild images — use build_ship for full pipeline.", {

...
run_all_checks() {
    ...
    /app/scripts/check-topology.sh >> "$REPORT_FILE"
...
generate_html_report() {
    ...
    echo "Generating HTML email template..."
    /app/scripts/generate-html.sh "$REPORT_FILE" "$REPORT_HTML"
    echo "HTML report generated: $REPORT_HTML"
}

# Send email with report (using swaks for attachment support)
send_email() {
    echo "Sending report via swaks with HTML email..."

    # SMTP credentials (Mailu relay account)
    SMTP_USER="${SMTP_USER:-no-reply@diegonmarcos.com}"
    SMTP_PASS="${SMTP_PASS:-}"
    REPORT_HTML="$REPORT_DIR/health-report-${TIMESTAMP}.html"

    # Primary: swaks via Mailu SMTPS (port 465 implicit TLS) with auth
    if [[ -n "$SMTP_PASS" ]]; then
        # Send HTML email with attachments (HTML body + MD/JSON/HTML attachments)
        if swaks --to "$TO_EMAIL" \
                 ...
                 --attach-name "health-report.html" \
                 --attach "@$REPORT_HTML" \
                 ...
                 --attach "@$REPORT_JSON" \
                 --timeout 30 2>&1; then
            echo "[OK] Email sent via Mailu SMTPS with HTML body and attachments (HTML + MD + JSON)"
            return 0
        fi
    fi
...
main() {
    ...
    echo "  - HTML: $REPORT_DIR/health-report-${TIMESTAMP}.html"
    echo ""

...
export function registerSpecTools(server: McpServer) {
  ...
  server.tool(
    "knowledge.spec",
    "Get service/VM spec. Methods: service (build.json+flake.nix+topology, requires name), vm (IP/WG/services/SSH alias, requires vm), services_by_category (all services grouped by category+VM+domain)",
    {
      method: z.enum(["service", "vm", "services_by_category"]),
      name: z.string().optional().describe("Service name (for service method)"),
      vm: z.string().optional().describe("VM ID or SSH alias (for vm method)"),
    },
    async ({ method, name, vm }) => {

      if (method === "service") {
        if (!name) {
          return { content: [{ type: "text" as const, text: `Missing required param: name` }] };
        }
      ...
      if (method === "vm") {
        if (!vm) {
          return { content: [{ type: "text" as const, text: `Missing required param: vm` }] };
        }
        const config = getConfig();

...
const TEMPLATE_FILE: &str = "cloud_health_full.md.tpl";
const OUTPUT_PATH: &str = "cloud_health_full.md";
...
/// Render template to a String (no write). Caller concatenates with stack output.
pub fn render_string(vars: &HashMap<String, String>) -> anyhow::Result<String> {
    ...
    println!("Template loaded ({} chars, {} vars) from {}", template.len(), vars.len(), tpl.display());
...
pub fn output_path() -> &'static str { OUTPUT_PATH }

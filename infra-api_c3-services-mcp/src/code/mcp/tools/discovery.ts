...
function loadTopology(): TopoData | null {
  ...
  const candidates = [
    ...
    join(gitBase, "cloud", "1_configs", "dist", "build-c3-services-mcp.json"),                    // dev / co-located clone
    "/app/_cloud-data-consolidated.json",                                                         // in-image fallback
    join(gitBase, "cloud", "1_configs", "dist", "_cloud-data-consolidated.json"),                 // dev fallback
  ];

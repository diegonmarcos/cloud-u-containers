...
rule Suspicious_Encoded_Payload {
    meta:
        description = "Detects heavily encoded/obfuscated content in scripts"
        severity = "medium"
    strings:
        ...
        $gzip_b64 = "H4sIA"
    condition:
        // Only flag if it's a script file with encoded content
        ($php or $bash or $sh or $python or $perl) and
        ($b64_long or $hex_long or $gzip_b64)
}

...
# Cypht's lib/config.php::load_user_config_object switches on 'DB'
# (uppercase). Anything else (including 'database', 'db', 'postgres') falls
# through to Hm_User_Config_File. Same for SESSION_TYPE in lib/session_base.php.
# Bug evidence: my server-side apply.php wrote 17 feeds + 9 settings to
# hm_user_settings.settings successfully, but the UI showed defaults
# because the framework was reading from a FILE (file-based config) instead
# of the DB. Verified via load_user_config_object in lib/config.php on
# upstream cypht-org/cypht.
SESSION_TYPE=DB
USER_CONFIG_TYPE=DB

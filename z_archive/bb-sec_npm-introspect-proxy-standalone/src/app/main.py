...
OIDC introspection endpoint. Used by NPM's auth_request directive to
authenticate CLI/API clients.

Architecture:
    CLI Client -> NPM (auth_request) -> This Proxy -> Authelia Introspection
                                              |
                                              v
...
def auth():
    ...
    Expected by nginx auth_request directive:
    - 2xx: Allow request, forward auth headers to backend
    - 401: Deny request (will trigger error_page fallback)

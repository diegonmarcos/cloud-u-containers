{
  ...
  outputs = { self, nixpkgs }: let
    ...
    _mkConfigTomlOld = pkgs: pkgs.writeText "config.toml.tpl.old" ''
      ...
      # fail2ban disabled — security handled at cloud level (Caddy + Authelia + firewalls)
      # Stalwart bug: allowed-ip-addresses doesn't exempt WG IPs from fail2ban blocks
    ...
    mkInitSh = pkgs: pkgs.writeText "init.sh" ''
      ...
      (sleep 10 && AUTH="admin@${config.domain}:$ADMIN_PASSWORD" && \
        curl -sk -X POST "https://localhost:443/api/principal" \

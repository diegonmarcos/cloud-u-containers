...
[session.rcpt]
directory = "'internal'"
# Port 25: inbound only (no relay). Ports 465/587: relay allowed (auth required).
relay = [{if = "local_port == 25", then = "false"}, {else = "true"}]
...
# Outbound relay for non-local mail: OCI primary, AWS fallback
# Local domains (in internal directory) auto-deliver to mailbox — no next-hop needed
[queue.outbound]
next-hop = [{if = "is_local_domain('internal', rcpt_domain)", then = "false"}, {if = "retry_num > 0", then = "'aws-relay'"}, {else = "'oci-relay'"}]

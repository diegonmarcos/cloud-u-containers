...
[ -f "$C3_MCP_SECRETS" ]   || { echo "FAIL: cloud-infra-mcp secrets missing"; exit 2; }
...
check_key "$C3_MCP_SECRETS"   "NOREPLY_PASSWORD"        "cloud-infra-mcp"
...
if [ "$MADDY_HASH" != "$C3_HASH" ]; then
    echo "FAIL: cloud-infra-mcp NOREPLY_PASSWORD does not match Maddy NOREPLY_PASSWORD"
    echo "  maddy sha256: $MADDY_HASH"
    echo "  c3    sha256: $C3_HASH"
    exit 1
fi
echo "  OK: cloud-infra-mcp and maddy NOREPLY passwords match"

echo ""
echo "PASS: NOREPLY secret is present + consistent across maddy, cloud-mail-mcp, cloud-infra-mcp"

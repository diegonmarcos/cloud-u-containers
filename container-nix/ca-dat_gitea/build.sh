...
step_compose() {
    ...
    ssh "$DEPLOY_HOST" "cd $DEPLOY_PATH && docker-compose down --remove-orphans 2>/dev/null; docker-compose \$([ -f .secrets ] && echo '--env-file .secrets') up -d --build --no-deps $SERVICE_NAME"

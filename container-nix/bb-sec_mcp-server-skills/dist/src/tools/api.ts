...
export function registerApiTools(server: McpServer) {
  server.tool(
    "api_call",
    "[DEPRECATED: use service_api_call or health_* tools] Call the legacy Flask API (api.diegonmarcos.com) endpoint",
    {
  ...
  server.tool(
    "api_vm_control",
    "[DEPRECATED: use vm_start, vm_stop, vm_reset] Start, stop, or reset a VM via OCI SDK / gcloud CLI",
    {

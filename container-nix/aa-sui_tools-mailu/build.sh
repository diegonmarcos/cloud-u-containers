...
step_deploy() {
    ...
    # Sync config files only (exclude Docker data dirs and certs on remote)
    EXCLUDES="--exclude=mail/** --exclude=mailqueue/** --exclude=data/** --exclude=dkim/** --exclude=filter/** --exclude=redis/** --exclude=webmail/** --exclude=certs/**"
    if command -v rsync >/dev/null 2>&1; then
        rsync -avz --delete $EXCLUDES "$DIST_DIR/" "$DEPLOY_HOST:$DEPLOY_PATH/"
...
step_compose() {
    ...
    # Post-start setup (aliases, etc.) — generated by flake
    log "Running post-start setup..."
    ssh "$DEPLOY_HOST" "cd $DEPLOY_PATH && chmod +x setup.sh && ./setup.sh"
}

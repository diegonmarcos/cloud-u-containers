{
  ...
  outputs = { self, nixpkgs }: let
    ...
    mailuEnv = pkgs.writeText "mailu.env" ''
      # Mailu configuration
  ...
  in {
    packages.${system} = {
      default = pkgs.runCommand "mailu-configs" {} ''
        mkdir -p $out/{certs,data,dkim,mail,mailqueue,filter,webmail,redis}
        mkdir -p $out/overrides/{nginx,dovecot,postfix,rspamd,roundcube}
        cp ${dockerCompose} $out/docker-compose.yml
        cp ${mailuEnv} $out/mailu.env
      '';
